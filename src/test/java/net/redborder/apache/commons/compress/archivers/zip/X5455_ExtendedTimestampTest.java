...
import static net.redborder.apache.commons.compress.AbstractTestCase.mkdir;
import static net.redborder.apache.commons.compress.AbstractTestCase.rmdir;
...
public class X5455_ExtendedTimestampTest {
    ...
    private void parseReparse(
        ...
        if (isFlagSet(expectedFlags, X5455_ExtendedTimestamp.MODIFY_TIME_BIT)) {
            assertTrue(xf.isBit0_modifyTimePresent());
            assertEquals(time, xf.getModifyTime());
        }
        if (isFlagSet(expectedFlags, X5455_ExtendedTimestamp.ACCESS_TIME_BIT)) {
            assertTrue(xf.isBit1_accessTimePresent());
            assertEquals(time, xf.getAccessTime());
        }
        if (isFlagSet(expectedFlags, X5455_ExtendedTimestamp.CREATE_TIME_BIT)) {
            assertTrue(xf.isBit2_createTimePresent());
            assertEquals(time, xf.getCreateTime());
        }
