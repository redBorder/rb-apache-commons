...
import net.redborder.apache.commons.compress.archivers.ArchiveOutputStream;
...
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.DATA_DESCRIPTOR_MIN_VERSION;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.DWORD;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.INITIAL_VERSION;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.SHORT;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.WORD;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.ZIP64_MAGIC;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.ZIP64_MAGIC_SHORT;
import static net.redborder.apache.commons.compress.archivers.zip.ZipConstants.ZIP64_MIN_VERSION;
import static net.redborder.apache.commons.compress.archivers.zip.ZipLong.putLong;
...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
    ...
    private byte[] createLocalFileHeader(ZipArchiveEntry ze, ByteBuffer name, boolean encodable,
        ...
        if (phased &&  !isZip64Required(entry.entry, zip64Mode)){
            ZipShort.putShort(INITIAL_VERSION, buf, LFH_VERSION_NEEDED_OFFSET);
        } else {
            ZipShort.putShort(versionNeededToExtract(zipMethod, hasZip64Extra(ze)), buf, LFH_VERSION_NEEDED_OFFSET);
        }
    ...
    private byte[] createCentralFileHeader(ZipArchiveEntry ze, ByteBuffer name, long lfhOffset,
        ...
        ZipShort.putShort((ze.getPlatform() << 8) | (!hasUsedZip64 ? DATA_DESCRIPTOR_MIN_VERSION : ZIP64_MIN_VERSION),
                buf, CFH_VERSION_MADE_BY_OFFSET);

        final int zipMethod = ze.getMethod();
        final boolean encodable = zipEncoding.canEncode(ze.getName());
        ZipShort.putShort(versionNeededToExtract(zipMethod, needsZip64Extra), buf, CFH_VERSION_NEEDED_OFFSET);
        ...
        // extra field length
        ZipShort.putShort(extra.length, buf, CFH_EXTRA_LENGTH_OFFSET);
